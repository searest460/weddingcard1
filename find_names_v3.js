...
while ((bytesRead = fs.readSync(fd, buffer, 0, 100000, pos)) > 0) {
    ...
    searchStrings.forEach(s => {
        ...
        while ((index = chunk.indexOf(s, index + 1)) !== -1) {
            results += `Found "${s}" at absolute index ${pos + index}\n`;
            results += `Context: ${chunk.substring(Math.max(0, index - 50), Math.min(chunk.length, index + 100))}\n\n`;
        }
    });
    pos += bytesRead - 100;
...
fs.closeSync(fd);
fs.writeFileSync('search_results.txt', results);
